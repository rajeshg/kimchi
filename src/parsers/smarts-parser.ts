...
export function parseSMARTS(pattern: string): { pattern: SMARTSPattern | null; errors: string[] } {
  const errors: string[] = [];
    ...
    if (char === '[') {
      ...
      if (lastAtomIndex >= 0) {
        bonds.push({
          from: lastAtomIndex,
          to: atomIndex,
          primitives: pendingBond || result.precedingBond || getImplicitBondType(atoms, lastAtomIndex, atomIndex)
        });
      ...
      const ringResult = parseRingClosures(pattern, i, atomIndex - 1, ringClosures, bonds, atoms, pendingBond);
      pendingBond = undefined;
  ...
  while (i < pattern.length && pattern[i] !== ']') {
    ...
    if (char === '!') {
      ...
      if (i >= pattern.length || pattern[i] === ']') {
        return { primitives: [], nextIndex: i, error: `Unexpected '!' at position ${i - 1}` };
      }
      ...
      if (nextChar === '#') {
        ...
        primitives.push({ type: 'atomic_number', value: numResult.value ?? 0, negate: true });
        tokens.push({ type: 'atomic_number', value: numResult.value ?? 0, negate: true });
        ...
        i++;
        const numResult = parseNumber(pattern, i);
        primitives.push({ type: 'degree', value: numResult.value || 1, negate: true });
        tokens.push({ type: 'degree', value: numResult.value || 1, negate: true });
        i = numResult.nextIndex;
      } else if (nextChar === 'X') {
        i++;
        const numResult = parseNumber(pattern, i);
        primitives.push({ type: 'connectivity', value: numResult.value || 1, negate: true });
        tokens.push({ type: 'connectivity', value: numResult.value || 1, negate: true });
        i = numResult.nextIndex;
      } else if (nextChar === 'H') {
        i++;
        const numResult = parseNumber(pattern, i);
        primitives.push({ type: 'total_h', value: numResult.value === null ? 1 : numResult.value, negate: true });
        tokens.push({ type: 'total_h', value: numResult.value === null ? 1 : numResult.value, negate: true });
        i = numResult.nextIndex;
      } else if (nextChar === 'R') {
        i++;
        const numResult = parseNumber(pattern, i);
        ...
        primitives.push({ type: 'element', value: element.value, negate: true });
        tokens.push({ type: 'element', value: element.value, negate: true });
        i = element.nextIndex;
      } else {
        return { primitives: [], nextIndex: i, error: `Unexpected character after '!': ${nextChar} at position ${i}` };
      }
    } else if (char === '#') {
      i++;
      const numResult = parseNumber(pattern, i);
      ...
      i++;
      const numResult = parseNumber(pattern, i);
      primitives.push({ type: 'total_h', value: numResult.value === null ? 1 : numResult.value });
      tokens.push({ type: 'total_h', value: numResult.value === null ? 1 : numResult.value });
      i = numResult.nextIndex;
    } else if (char === 'h') {
      i++;
      const numResult = parseNumber(pattern, i);
      primitives.push({ type: 'implicit_h', value: numResult.value === null ? 1 : numResult.value });
      tokens.push({ type: 'implicit_h', value: numResult.value === null ? 1 : numResult.value });
      i = numResult.nextIndex;
    } else if (char === 'R') {
      i++;
      const numResult = parseNumber(pattern, i);
      ...
      i++;
      const numResult = parseNumber(pattern, i);
      primitives.push({ type: 'ring_connectivity', value: numResult.value || 1 });
      ...
      return { primitives: [], nextIndex: i, error: `Unexpected character in bracket atom: ${char} at position ${i}` };
    }
  }
  
  if (i >= pattern.length) {
    return { primitives: [], nextIndex: i, error: `Unclosed bracket atom starting at position ${startIndex}` };
  }
...
function parseRingClosures(
    ...
    if (i < pattern.length && isDigit(pattern[i]!)) {
      ...
      if (ringClosures.has(digit)) {
        ...
        bonds.push({
          ...
          to: atomIndex,
          primitives: ringBond || closure.bond || getImplicitBondType(atoms, closure.atomIndex, atomIndex),
          isRingClosure: true
        });
  ...
  if (i < pattern.length && pattern[i] === '%') {
    ...
    if (i + 1 < pattern.length && isDigit(pattern[i]!) && isDigit(pattern[i + 1]!)) {
      ...
      return { nextIndex: i, error: `Invalid ring closure: % must be followed by two digits at position ${i - 1}` };
    }
  }
...
function parseNumber(pattern: string, startIndex: number): { value: number | null; nextIndex: number } {
  let i = startIndex;
  let numStr = '';
...
function parseBondSymbol(char: string): BondPrimitive[] {
  switch (char) {
    case '-': return [{ type: 'single' }];
    case '=': return [{ type: 'double' }];
    case '#': return [{ type: 'triple' }];
    case ':': return [{ type: 'aromatic' }];
