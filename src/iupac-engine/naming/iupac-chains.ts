...
function getFunctionalGroupPositions(
  chain: number[],
  molecule: Molecule,
): number[] {
...
function isBetterPriorityLocants(
...
function isBetterByOpsinHeuristics(
  molecule: Molecule,
  aChain: number[],
  bChain: number[],
): boolean {
...
function renumberPriorityLocants(
  ...
  for (const atomIdx of mainChain) {
    ...
    switch (atom.symbol) {
      ...
      case "N":
        ...
        break;
      case "S":
        prefix = `${position}-thia`;
        break;
      case "P":
        prefix = `${position}-phospha`;
        break;
      case "Si":
        prefix = `${position}-sila`;
        break;
      case "B":
        prefix = `${position}-bora`;
        break;
      case "F":
        prefix = `${position}-fluora`;
        break;
      case "Cl":
        prefix = `${position}-chlora`;
        break;
      case "Br":
        prefix = `${position}-broma`;
        break;
      case "I":
        prefix = `${position}-ioda`;
        break;
      default:
        prefix = `${position}-${atom.symbol.toLowerCase()}a`;
    }
...
function nameComplexAlkoxySubstituent(
  ...
  function collectLinker(currentIdx: number): void {
    ...
    if (
      substituentAtoms.has(currentIdx) &&
      molecule.atoms[currentIdx]?.symbol === "C"
    ) {
  ...
  function collectTail(currentIdx: number): void {
    ...
    for (const bond of molecule.bonds) {
      ...
      if (
        neighborIdx >= 0 &&
        !tailVisited.has(neighborIdx) &&
        !linkerAtoms.has(neighborIdx)
      ) {
  ...
  if (process.env.VERBOSE) {
    console.log(
      `  Complex ether debug: primaryO=${primaryOxygenIdx}, nestedO=${firstNestedOxygen}, tailStartC=${tailStartCarbon}`,
    );
    console.log(
 ...
 */
function determineRingAttachmentPosition(
  ring: readonly number[],
