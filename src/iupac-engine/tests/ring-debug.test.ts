...
import { analyzeRings, findRings, findSSSR } from '../../utils/ring-analysis';
  ...
  it('prints ring detection info for sample SMILES', () => {
    for (const c of cases) {
      try {
        ...
        console.log('  parser.rings:', Array.isArray(mol.rings) ? mol.rings.map(r => r.length) : mol.rings);
        // Atom aromatic flags
        const aromaticAtoms = mol.atoms.filter(a => a.aromatic).map(a => a.id);
        console.log('  aromatic atom ids:', aromaticAtoms);
        // SSSR via analyzeRings
        const ringInfo = analyzeRings(mol as any);
        console.log('  SSSR count:', ringInfo.rings.length, 'SSSR sizes:', ringInfo.rings.map(r => r.length));
        // All elementary rings
        const all = findRings(mol.atoms as any, mol.bonds as any);
        console.log('  all cycles count:', all.length, 'sizes:', all.map(r => r.length));
        // MCB/SSSR via findSSSR
        const sssr = findSSSR(mol.atoms as any, mol.bonds as any);
        console.log('  findSSSR count:', sssr.length, 'sizes:', sssr.map(r => r.length));
      } catch (err) {
