 ...
 */
function detectAmideGroup(atomId: number, molecule: Molecule, visited: Set<number> = new Set()): { carbonylC: number, nitrogen: number } | null {
  ...
  if (atom.symbol === 'C') {
    ...
    for (const bond of molecule.bonds) {
      if (bond.type === 'double') {
        if ((bond.atom1 === atomId && molecule.atoms[bond.atom2]?.symbol === 'O') ||
    ...
    for (const bond of molecule.bonds) {
      if (bond.type === 'single') {
        ...
        if (neighborId !== undefined) {
          ...
          if (process.env.VERBOSE) {
            console.log(`[detectAmideGroup] Neighbor ${neighborId}: ${neighbor?.symbol || 'undefined'}`);
          }
    ...
    if (process.env.VERBOSE) {
      console.log(`[detectAmideGroup] No amide found from atom ${atomId}, checked ${neighbors.length} neighbors`);
    }
  }
...
function buildComplexAlkoxyWithAmide(
  ...
  const substituents: Array<{ type: string, name: string, locant: number, atomId: number }> = [];
...
function findRingSubstituents(
  ...
  for (const bond of molecule.bonds) {
    if (bond.type === 'aromatic' || (bond.type === 'single' && molecule.atoms[bond.atom1]?.aromatic && molecule.atoms[bond.atom2]?.aromatic)) {
      ...
      if (neighbor.symbol === 'N' && neighbor.charge === 1) {
        ...
        for (const b2 of molecule.bonds) {
          if (b2.type === 'single') {
            ...
            else if (b2.atom2 === neighborId && molecule.atoms[b2.atom1]?.symbol === 'F') fId = b2.atom1;
...
export function getAlkoxyGroupName(esterGroup: any, molecule: Molecule): string {
  ...
  if (amideCheck) {
    ...
    const complexName = buildComplexAlkoxyWithAmide(alkoxyCarbonId, esterOxygenId, amideCheck, molecule);
    if (complexName) {
      return complexName;
    }
          ...
          for (const bond2 of molecule.bonds) {
            ...
            if (siliconId !== undefined) {
              ...
              for (const bond3 of molecule.bonds) {
                ...
                } else if (bond3.atom2 === siliconId && molecule.atoms[bond3.atom1]?.symbol === 'C') {
                  carbonIdOnSi = bond3.atom1;
                }
                
                if (carbonIdOnSi !== undefined) {
                  // Check if this carbon is a methyl (no other carbons attached)
                  const carbonNeighbors = molecule.bonds.filter((b: any) => 
              ...
              if (process.env.VERBOSE) {
                console.log(`[getAlkoxyGroupName] Found ${silyloxyName} at position ${i + 1} (carbon ${carbonId})`);
              }
            }
          }
        }
