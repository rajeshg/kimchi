...
export function detectRingSystems(molecule: any): any[] {
  ...
  for (const [_root, ringIndices] of groups) {
    ...
    const bonds = Array.from(bondSet).map(key => {
      const [a1, a2] = key.split('-').map(Number);
      return molecule.bonds.find((b: any) => 
 ...
 */
export function generateBridgedPolycyclicName(bridgedRings: number[][], molecule: any): { name: string; vonBaeyerNumbering?: Map<number, number> } | null {
 ...
 */
export function generateSpiroPolycyclicName(spiroRings: number[][], molecule: any): string | null {
  // Use the engine's own naming function
 ...
 */
export function generateFusedPolycyclicName(fusedRings: number[][], molecule: any): string | null {
