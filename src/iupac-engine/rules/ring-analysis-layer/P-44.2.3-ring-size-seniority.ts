...
export const P44_2_3_RING_SIZE_SENIORITY_RULE: IUPACRule = {
  ...
  conditions: (context) => {
    ...
    if (process.env.VERBOSE) {
      console.log(`[P-44.2.3 conditions] candidateRings: ${candidateRings?.length}, shouldApply: ${shouldApply}`);
    }
  ...
  action: (context) => {
    ...
    if (hasConnectedRings) {
      if (process.env.VERBOSE) {
        console.log(`[P-44.2.3] Rings are connected - keeping all ${candidateRings.length} rings as polycyclic parent`);
      }
      return context;
    }
    ...
    if (process.env.VERBOSE) {
      ...
      console.log('[P-44.2.3] FG types:', principalFGs.map((fg: any) => fg.type));
    }
    ...
    const ringFGScores = candidateRings.map((ring: any) => {
      ...
      if (process.env.VERBOSE) {
        console.log(`[P-44.2.3] Checking ring with ${ring.size} atoms, IDs:`, Array.from(ringAtomIds));
      }
      ...
      if (process.env.VERBOSE) {
        console.log(`[P-44.2.3]   Ring score: fgCount=${fgCount}, highestPriority=${highestPriority}, size=${ring.size}`);
      }
    ...
    if (maxFGCount > 0) {
      ...
      const ringsWithHighestPriorityFG = ringsWithFGs.filter(r => r.highestPriority === highestPriority);
      ...
      if (process.env.VERBOSE) {
        console.log(`[P-44.2.3] Selected ${selectedRings.length} ring(s) with FG count=${maxFGCount}, priority=${highestPriority}, size=${smallestSize}`);
      }
