...
export const P2_1_HETEROATOM_PARENT_HYDRIDE_RULE: IUPACRule = {
  ...
  conditions: (context: ImmutableNamingContext) => {
    ...
    if (process.env.VERBOSE) console.log('P-2.1: Found heteroatoms:', heteroatoms.map(a => a.symbol));

    // Must have exactly one heteroatom
    if (heteroatoms.length !== 1) {
      if (process.env.VERBOSE) console.log('P-2.1: Expected 1 heteroatom, found:', heteroatoms.length);
      return false;
    }
    ...
    if (process.env.VERBOSE) {
      console.log(`P-2.1: Heteroatom ${heteroatom.symbol}: bondOrders=${bondOrders}, implicitH=${implicitHydrogens}, total=${totalValence}, expected=${hydrideDef.valence}`);
    }
  ...
  action: (context: ImmutableNamingContext) => {
    const molecule = context.getState().molecule;
    const heteroatom = molecule.atoms.find(atom =>
      atom.symbol !== 'C' && atom.symbol !== 'H' && HETEROATOM_PARENT_HYDRIDES[atom.symbol]
    )!;
...
export const P2_2_CARBON_PARENT_HYDRIDE_RULE: IUPACRule = {
  ...
  conditions: (context: ImmutableNamingContext) => {
    ...
    console.error(`P2_2 conditions: functionalGroups.length = ${functionalGroups ? functionalGroups.length : 'undefined'}`);
    ...
    return carbons.length <= 10 && molecule.bonds.every(bond => bond.type === 'single') &&
           candidateChains && candidateChains.length === 1;
  },
  action: (context: ImmutableNamingContext) => {
    const molecule = context.getState().molecule;
    ...
    const defaultAlkaneNames = ['', 'methane', 'ethane', 'propane', 'butane', 'pentane',
                        'hexane', 'heptane', 'octane', 'nonane', 'decane'];
    let name = `${carbonCount}-ane`;
    try {
      const alkaneKey = 'C'.repeat(Math.max(1, carbonCount));
      const stem = OPSIN_RULES && OPSIN_RULES.alkanes && OPSIN_RULES.alkanes[alkaneKey];
      if (stem) {
        name = `${stem}ane`;
      ...
      // fallback
  if (carbonCount < defaultAlkaneNames.length) name = defaultAlkaneNames[carbonCount] || `${carbonCount}-ane`;
