  ...
  action: (context: ImmutableNamingContext) => {
    ...
    const assembledGroups = functionalGroups.map((group: FunctionalGroup) => {
      ...
      if (process.env.VERBOSE) {
        console.log(`[LOCANT_ASSEMBLY] Processing group: type=${group.type}, prefix=${group.prefix}, isPrincipal=${group.isPrincipal}, locants array=${JSON.stringify(group.locants)}, locants string=${locants}`);
      }
...
function buildSubstitutiveName(parentStructure: any, functionalGroups: any[]): string {
  ...
  if (parentStructure.type === 'ring' && parentStructure.ring?.atoms) {
    ...
    if (!Array.isArray(parentStructure.ring.atoms)) {
      if (process.env.VERBOSE) {
        console.warn(`[buildSubstitutiveName] WARNING: parentStructure.ring.atoms is not an array`);
      }
    } else {
      for (const atom of parentStructure.ring.atoms) {
        ...
        if (atom && typeof atom === 'object' && 'id' in atom) {
          ...
          if (process.env.VERBOSE) {
            console.warn(`[buildSubstitutiveName] WARNING: Unexpected atom structure in ring.atoms:`, atom);
          }
        }
      }
      if (process.env.VERBOSE && parentRingAtomIds.size > 0) {
        console.log(`[buildSubstitutiveName] Parent ring atom IDs:`, Array.from(parentRingAtomIds));
      }
    }
  }
  ...
  const fgSubstituentFilteredByAtoms = fgSubstituents.filter((fgSub: any) => {
    ...
    for (const atom of fgSub.atoms) {
      if (atom && typeof atom === 'object' && 'id' in atom) {
        ...
        // GUARD: Log warning if atom structure is unexpected
        if (process.env.VERBOSE) {
          console.warn(`[buildSubstitutiveName] WARNING: Unexpected atom structure in fgSub.atoms:`, atom);
        }
      }
    }
    ...
    if (hasOverlap) {
      if (process.env.VERBOSE) {
        console.log(`[buildSubstitutiveName] Filtering out FG "${fgSub.type}" (atoms: ${fgAtomIds}) - overlaps with parent ring atoms (ring: ${Array.from(parentRingAtomIds)})`);
      }
  ...
  if (parentStructureName && typeof parentStructureName === 'string') {
    for (const fgSub of fgSubstituentFilteredByAtoms) {
      ...
      if (parentStructureName.includes(fgType) || (fgPrefix && parentStructureName.includes(fgPrefix))) {
        fgTypesToFilter.add(fgType);
        if (process.env.VERBOSE) {
          console.log(`[buildSubstitutiveName] FG type "${fgType}" already in parent structure name "${parentStructureName}"`);
        }
      }
    }
  }
  ...
  for (const fgSub of fgSubstituentFilteredByAtoms) {
    ...
    for (const parentSubName of parentSubstituentNames) {
      ...
      if (parentSubName.length > 10 && (parentSubName.includes(fgType) || (fgPrefix && parentSubName.includes(fgPrefix)))) {
        fgTypesToFilter.add(fgType);
        if (process.env.VERBOSE) {
          console.log(`[buildSubstitutiveName] FG type "${fgType}" already in parent substituent "${parentSubName}"`);
        }
  ...
  const fgSubstituentsFinal = fgSubstituentFilteredByAtoms.filter((fgSub: any) => {
    // GUARD: Validate fgSub structure
    if (!fgSub || typeof fgSub !== 'object' || typeof fgSub.type !== 'string') {
      if (process.env.VERBOSE) {
        console.warn(`[buildSubstitutiveName] WARNING: Invalid fgSub structure, keeping it:`, fgSub);
      }
  ...
  const deduplicatedParentSubs = parentSubstituents.filter((pSub: any) => {
    ...
    if (locant !== undefined && type) {
      ...
      if (fgLocantTypeMap.has(key)) {
        if (process.env.VERBOSE) {
          console.log(`[buildSubstitutiveName] deduplicating parent substituent ${type} at locant ${locant} - already in FG substituents`);
        }
        return false;
      }
    }
  ...
  if (process.env.VERBOSE) {
    ...
    console.log('[buildSubstitutiveName] fgSubstituentsFinal:', JSON.stringify(fgSubstituentsFinal.map((s: any) => ({ type: s.type, name: s.name, locant: s.locant }))));
    ...
    console.log('[buildSubstitutiveName] deduplicatedParentSubs:', JSON.stringify(deduplicatedParentSubs.map((s: any) => ({ type: s.type, name: s.name, locant: s.locant }))));
  ...
  if (allSubstituents.length > 0) {
    ...
    if (isHeteroatomParent) {
      ...
      for (const sub of allSubstituents) {
        ...
        if (sub.suffix && sub.isPrincipal) continue;
