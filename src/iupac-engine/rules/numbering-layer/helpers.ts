...
export function normalizeFunctionalGroupLocants(functionalGroups: FunctionalGroup[], parentStructure: ParentStructure): FunctionalGroup[] {
...
export function optimizeLocantSet(parentStructure: ParentStructure, principalGroup: FunctionalGroup): number[] {
...
export function getPrincipalGroupLocant(parentStructure: ParentStructure, principalGroup: FunctionalGroup): number {
...
export function getPrincipalGroupLocantFromSet(parentStructure: ParentStructure, principalGroup: FunctionalGroup, locantSet: number[]): number {
...
export function calculateLocantScore(parentStructure: ParentStructure, locants: number[], principalGroup: FunctionalGroup): number {
...
export function findOptimalRingNumbering(ring: any, molecule: any, functionalGroups?: any[]): number {
  ...
  if (functionalGroups && functionalGroups.length > 0) {
    ...
    console.log(`[Ring Numbering] Principal groups found:`, principalGroups.map(g => `${g.type}(isPrincipal:${g.isPrincipal}, priority:${g.priority})`));
        ...
        for (const groupAtom of group.atoms) {
          ...
          for (let i = 0; i < ring.atoms.length; i++) {
            if (ring.atoms[i]?.id === groupAtomId) {
              ...
              console.log(`[Ring Numbering] Principal group ${group.type} is ring atom at position ${i} (atom ${groupAtomId})`);
  ...
  console.log(`[Ring Numbering] Principal group positions:`, Array.from(principalGroupPositions).sort((a, b) => a - b));
  ...
  for (let start = 0; start < ring.atoms.length; start++) {
    for (const direction of [1, -1]) {
      ...
      console.log(`[Ring Numbering] Starting at position ${start} (${directionStr}): principal = [${principalLocants.join(', ')}], substituent = [${substituentLocants.join(', ')}], all = [${allLocants.join(', ')}]`);
      ...
      if (bestLocants.length === 0) {
        ...
        if (principalLocants.length > 0 && bestPrincipalLocants.length > 0) {
          ...
          if (principalComparison < 0) {
            ...
            const locantComparison = compareLocantSets(substituentLocants, bestLocants.filter(locant => !bestPrincipalLocants.includes(locant)));
...
export function findOptimalRingNumberingFromHeteroatom(ring: any, molecule: any, heteroatomIndex: number, functionalGroups?: any[]): number {
  if (!ring || !ring.atoms || ring.atoms.length === 0) {
    return -1;
  }
...
export function findRingStartingPosition(ring: any, molecule?: any, functionalGroups?: any[]): number {
...
export function reorderRingAtoms(atoms: any[], startingPosition: number): any[] {
...
export function assignLowestAvailableLocant(usedLocants: number[], preferredLocant: number, index: number): number {
...
export function assignSubstituentLocants(group: FunctionalGroup, parentStructure: ParentStructure, _index: number): number[] {
...
export function validateNumbering(parentStructure: ParentStructure, functionalGroups: FunctionalGroup[]): { isValid: boolean; errors: string[] } {
  const errors: string[] = [];
