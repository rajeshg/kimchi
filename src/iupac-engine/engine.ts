...
context = context.withConflict({
  ...
  context: {}
}, `layer-${layerName}`, layer.name, '', ExecutionPhase.PARENT_STRUCTURE, `Layer ${layerName} dependencies not satisfied`);
  continue;
}
...
context = this.executeLayer(layer, context);
        ...
        // Check for conflicts
const hasConflicts = context.getHistory().some(trace => trace.conflicts && trace.conflicts.length > 0);
if (hasConflicts) {
   ...
   */
  generateNameWithContext(molecule: Molecule): { result: NamingResult; context: ImmutableNamingContext } {
    let context = ImmutableNamingContext.create(molecule);
   ...
   */
  private checkLayerDependencies(layer: Layer, context: ImmutableNamingContext): boolean {
    ...
    if (this.canExecuteRule(rule, updatedContext)) {
      try {
        ...
        updatedContext = updatedContext.withConflict({
          ...
          context: { error, rule }
        }, rule.id, rule.name, rule.blueBookReference, ExecutionPhase.PARENT_STRUCTURE, `Rule ${rule.id} execution error`);
      }
    }
  }
...
private handleConflicts(context: ImmutableNamingContext): void {
   ...
   */
  private buildCarboxylicAcidName(context: ImmutableNamingContext, group: any): string {
   ...
   * Build alcohol name
   */
  private buildAlcoholName(context: ImmutableNamingContext, group: any): string {
    const carbonCount = context.getState().molecule.atoms.filter(a => a.symbol === 'C').length;
  ...
  private buildAmineName(context: ImmutableNamingContext, group: any): string {
    const carbonCount = context.getState().molecule.atoms.filter(a => a.symbol === 'C').length;
    const amineNames = ['', 'methanamine', 'ethanamine', 'propanamine', 'butanamine', 'pentanamine'];
    let baseName: string;
    if (carbonCount <= 5) {
  ...
  private calculateConfidence(context: ImmutableNamingContext): number {
    ...
    const conflictCount = context.getHistory().reduce((acc, trace) => acc + (trace.conflicts ? trace.conflicts.length : 0), 0);
confidence -= conflictCount * 0.1;
   ...
   */
  private generateFallbackName(context: ImmutableNamingContext, error: any): NamingResult {
    return {
