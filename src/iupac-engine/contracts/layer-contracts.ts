  ...
  validationRules: [
    ...
    },
    {
      ...
      check: (context) => {
        ...
        return context.functionalGroups.every((group: any) => 
          group.priority >= principal.priority
        );
      },
      severity: 'error'
    }
...
export const PARENT_STRUCTURE_CONTRACT: LayerContract = {
  ...
  validationRules: [
    ...
    },
    {
      ...
      check: (context) => {
        if (!context.parentStructure || context.parentStructure.type !== 'chain') {
  ...
  provides: [
    {
      ...
      validation: (data) => data && typeof data === 'object' && Array.isArray(data.locants)
    },
    {
      name: 'locantAssignments',
      type: 'Map<string, number>',
      description: 'Mapping of substituent names to their locants',
      validation: (data) => data instanceof Map || (data && typeof data === 'object')
    }
  ],
  validationRules: [
    {
...
export class ContractValidator {
   ...
   */
  private static checkDependency(dep: DependencyRequirement, context: any): boolean {
   ...
   */
  private static checkOutput(output: DataStructureDefinition, context: any): boolean {
