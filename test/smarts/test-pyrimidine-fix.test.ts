...
describe("Pyrimidine SMARTS Test - Aromaticity Case Sensitivity", () => {
  it("should match [n&R&D2] (aromatic N) in pyrimidine (c1ncccn1)", async () => {
    ...
    if (process.env.RUN_VERBOSE) console.log("Pattern:", JSON.stringify(smartsPattern.pattern, null, 2));
    
    // Match pattern against molecule
    const matches = matchSMARTS(smartsPattern.pattern!, mol, { uniqueMatches: true });
    
    if (process.env.RUN_VERBOSE) console.log("\n=== OPENCHEM ANALYSIS ===");
    if (process.env.RUN_VERBOSE) console.log("Pyrimidine SMILES: c1ncccn1");
    if (process.env.RUN_VERBOSE) console.log("Pattern: [n&R&D2]");
    if (process.env.RUN_VERBOSE) console.log("Matches:", matches.matches.length);
    
    // Check atom properties in detail
    if (process.env.RUN_VERBOSE) console.log("\nAtom properties:");
    mol.atoms.forEach((atom, idx) => {
      if (process.env.RUN_VERBOSE) console.log(`  Atom ${idx}: ${atom.symbol}`);
      if (process.env.RUN_VERBOSE) console.log(`    aromatic: ${atom.aromatic}`);
      if (process.env.RUN_VERBOSE) console.log(`    degree: ${atom.degree}`);
      if (process.env.RUN_VERBOSE) console.log(`    isInRing: ${atom.isInRing}`);
      if (process.env.RUN_VERBOSE) console.log(`    ringIds: ${JSON.stringify(atom.ringIds)}`);
    });
    
    // Check bonds
    if (process.env.RUN_VERBOSE) console.log("\nBonds:");
    mol.bonds.forEach((bond, idx) => {
      if (process.env.RUN_VERBOSE) console.log(`  Bond ${idx}: ${bond.atom1} -> ${bond.atom2}, type: ${bond.type}`);
    });
    ...
    try {
      ...
      const rdkitMatches = JSON.parse(rdkitMol.get_substruct_matches("[n&R&D2]"));
      if (process.env.RUN_VERBOSE) console.log("RDKit matches:", rdkitMatches);
      if (process.env.RUN_VERBOSE) console.log("RDKit match count:", rdkitMatches.length);
      
      // Get canonical SMILES from RDKit to see aromaticity
      const canonSmiles = rdkitMol.get_smiles();
      if (process.env.RUN_VERBOSE) console.log("RDKit canonical SMILES:", canonSmiles);
      
      rdkitMol.delete();
    } catch (err) {
      if (process.env.RUN_VERBOSE) console.log("RDKit comparison skipped:", err);
    }
  ...
  it("should NOT match [N&R&D2] (aliphatic N) in pyrimidine - expected behavior", () => {
    ...
    if (process.env.RUN_VERBOSE) console.log("Pyrimidine SMILES: c1ncccn1");
    if (process.env.RUN_VERBOSE) console.log("Pattern: [N&R&D2] (uppercase = aliphatic)");
    if (process.env.RUN_VERBOSE) console.log("Matches:", matches.matches.length);
    if (process.env.RUN_VERBOSE) console.log("Expected: 0 (pyrimidine has aromatic nitrogens)");
