...
describe('Functional group locants and trace metadata', () => {
  ...
  expect(result.parentStructure).toBeDefined();
    ...
    // Find alcohol FG
    const alcohol = result.functionalGroups.find(g => /alcohol|hydroxyl|alcoh/.test(g.type || (g.assembledName || '')) || (g.suffix && g.suffix.includes('ol')));
    expect(alcohol).toBeDefined();

    // All locants should be numeric; if parent locants are available, they should map within range
    const parentSize = (result.parentStructure?.locants && result.parentStructure.locants.length > 0)
  ...
  expect(result.parentStructure).toBeDefined();
    ...
    const acid = result.functionalGroups.find(g => /acid|carboxylic/.test(g.type || (g.assembledName || '')) || (g.suffix && /acid|oic/.test(g.suffix)));
    ...
    if ((result.parentStructure?.locants || []).length > 0) {
      expect(acid?.locants.every(l => l >= 1 && l <= result.parentStructure!.locants.length)).toBeTruthy();
    }
