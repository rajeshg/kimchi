...
describe('Blue Book P-44.3.6 Example', () => {
  ...
  test('should demonstrate immutable transitions preserve debugging information', () => {
    ...
    history.forEach(trace => {
      ...
      console.log(`  Confidence change: ${trace.beforeState.confidence} → ${trace.afterState.confidence}`);
    });
  });
});
  ...
  return [
    {
      ...
      length: 4,
      multipleBonds: [],
      substituents: [{ locant: 2, type: 'methyl' }, { locant: 3, type: 'methyl' }],
      locants: [1, 2, 3, 4]
    },
    {
      ...
      length: 6,
      multipleBonds: [],
      substituents: [{ locant: 2, type: 'methyl' }, { locant: 3, type: 'methyl' }, { locant: 4, type: 'methyl' }],
      locants: [1, 2, 3, 4, 5, 6]
    }
  ];
}
