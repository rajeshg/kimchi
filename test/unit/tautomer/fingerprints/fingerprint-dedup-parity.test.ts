...
describe('tautomer: fingerprint dedup parity', () => {
  it('produces identical unique SMILES sets when dedup enabled vs disabled', () => {
    ...
    for (const smi of cases) {
      ...
      if (!parsed || !parsed.molecules || parsed.molecules.length === 0) throw new Error('parse failed: ' + smi);
      const mol = parsed.molecules[0]!;

      const tNoFp = enumerateTautomers(mol, { maxTautomers: 512, phases: [1,2,3], useFingerprintDedup: false });
      const tWithFp = enumerateTautomers(mol, { maxTautomers: 512, phases: [1,2,3], useFingerprintDedup: true });
