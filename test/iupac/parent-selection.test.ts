...
describe('IUPAC parent selection (representative difficult cases)', () => {
  const cases: Array<{smiles: string; expectChainLength?: number; expectNameContains?: string; expectFullName?: string}> = [
    ...
    { smiles: 'C1CC2C3CCCCC3CC2C1', expectNameContains: 'tricyclo' },

    { smiles: 'C1=CC=C(C=C1)C2=CC=CC=C2', expectNameContains: 'biphenyl' },
    { smiles: 'CC(C1=CC=CC=C1)C2=CC=CC=C2', expectNameContains: 'biphenyl' },
    { smiles: 'CC1=CC=CC=C1C(C)C', expectNameContains: 'benz' },
    { smiles: 'CC1CC(C)C(C(C)C)C1', expectNameContains: 'cyclohexan' },
    { smiles: 'C1CC(C2CCCCC2)CCCC1', expectNameContains: 'bicyclo' },

    { smiles: 'CC(C(=O)O)CC(=O)O', expectChainLength: 4 },
    { smiles: 'OC(=O)CC(=O)O', expectChainLength: 2 },
    { smiles: 'CC(C(=O)O)C(=O)O', expectChainLength: 3 },
    { smiles: 'CCC(=O)C(=O)O', expectChainLength: 4 },
    { smiles: 'CC(C(=O)OC)C(=O)O', expectChainLength: 4 },

    { smiles: 'CCN(CC)CC', expectChainLength: 2 },
    { smiles: 'CC(=O)NC(=O)C', expectChainLength: 3 },
    { smiles: 'CC(=O)OC(=O)C', expectChainLength: 3 },
    { smiles: 'CC(C)(C(=O)O)C(=O)O', expectChainLength: 3 },

    { smiles: 'C1CCC(C(C)C(=O)O)CC1', expectNameContains: 'cyclohexan' },
    ...
    { smiles: 'C1=CC=C2C3=CC=CC=C3C=C12', expectNameContains: 'anthracene' },
    { smiles: 'C1=CC=C2C3=CC=CC=C3C=C2C=C1', expectNameContains: 'phenanthrene' },

    { smiles: 'CC(C(=O)O)C1=CC=CC=C1', expectNameContains: 'benz' },
    { smiles: 'CC(C)C1=CC=CC=C1C(=O)O', expectNameContains: 'benzo' },
  ];
